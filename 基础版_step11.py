            ...
            with self.db.cursor() as cur:
                cur.execute(
                    ...
                    DO UPDATE SET pacing_mode=EXCLUDED.pacing_mode,
                                  ...
                                  last_user_message_at=CASE WHEN %s THEN NOW() ELSE conversation_pacing_state.last_user_message_at END,
                                  last_ai_message_at=CASE WHEN %s THEN NOW() ELSE conversation_pacing_state.last_ai_message_at END,
                                  notes_json=EXCLUDED.notes_json,
                                  updated_at=NOW()
                    """,
                    (
                        conversation_id, pacing_mode, boundary_signal, pressure_level,
                        next_followup_after, followup_snooze_until, boundary_signal,
...
class SenderService:
    def __init__(self, outbound_job_repo: OutboundJobRepository) -> None:
    ...
    def send_text_reply(self, conversation_id: int, text: str, delay_seconds: int = 0,
        ...
        self.outbound_job_repo.enqueue(
            ...
            payload={
                ...
                "text": text,
                "delivery_plan": delivery_plan or {"plan_version": 1, "steps": [], "summary": ""},
            },
...
def build_app_components(settings: Settings) -> dict[str, Any]:
    ...
    sender_service = SenderService(outbound_job_repo)
                ...
                cur.execute(
                    ...
                    SET detected_language=%s,
                        ...
                        language_source=CASE
                            WHEN reply_language_mode='auto_follow' AND %s IS NOT NULL THEN 'auto_detected'
                            ELSE language_source
                        END,
                        preferred_language=CASE
                            WHEN reply_language_mode='auto_follow' AND %s IS NOT NULL THEN %s
                            ELSE preferred_language
                        END,
                        updated_at=NOW()
                    WHERE id=%s
                    """,
                    (normalized_language, confidence, normalized_language, normalized_language, normalized_language, user_id),
                )
