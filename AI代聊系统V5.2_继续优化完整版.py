...
class ConversationStageEngine(ConversationStageEngine):
    def decide(self, understanding: UnderstandingResult, current_stage: str | None) -> StageDecision:
...
class ReplyStyleEngine(ReplyStyleEngine):
    def generate(self, latest_user_message: str, recent_context: list[dict], persona_summary: str, user_state_summary: str, stage: str, chat_mode: str, understanding: dict, reply_plan: dict, selected_content: dict) -> str:
...
class ReplySelfCheckEngine(ReplySelfCheckEngine):
    def check_and_fix(self, draft_reply: str, mode: str, understanding: UnderstandingResult) -> str:
...
class ReplyDelayEngine(ReplyDelayEngine):
    def decide_delay_seconds(self, understanding: UnderstandingResult, mode: str, reply_text: str) -> int:
        _ = reply_text
        if understanding.boundary_signal:
            return 1
...
class AsyncSenderWorker(AsyncSenderWorker):
...
class TelegramUpdateMapper(TelegramUpdateMapper):
    def map_update(self, raw_update: dict) -> InboundMessage | None:
        message_obj = None
        update_type = None
        if raw_update.get("business_message"):
...
class SenderService(SenderService):
...
class ConversationRepository(ConversationRepository):
...
class DashboardService(DashboardService):
    def __init__(self, *args, decision_audit_repo: DecisionAuditRepository | None = None, override_lock_repo: OverrideLockRepository | None = None, **kwargs) -> None:
        super().__init__(*args, **kwargs)
        self.decision_audit_repo = decision_audit_repo
        self.override_lock_repo = override_lock_repo
...
class ProjectClassifier(ProjectClassifier):
...
# V5.2 patched: defer process entry until the true end of file.
...
# =========================
# V5.2.1 deployment hardening patch
# =========================

def _v521_runtime_sanity_check():
    """Lightweight startup sanity check for direct deployment builds."""
    required = [
        'BOT_TOKEN',
        'DATABASE_URL',
    ]
    ...
    if missing:
        logger.warning("运行时环境变量缺失: %s", ", ".join(missing))
    return missing

_original_main = main

def main():
    _v521_runtime_sanity_check()
