...
# ======== V5.2 stabilization patch: final chain convergence, admin menu upgrade, nurture compat ========

_V52CompatPrevProjectNurturePlanner = ProjectNurturePlanner
class ProjectNurturePlanner(_V52CompatPrevProjectNurturePlanner):
    def plan(self, *args, **kwargs) -> dict[str, Any]:
        """
        Compatibility wrapper for all historical call signatures.
        Supports:
        - plan(understanding, stage_decision, turn_decision, project_decision, user_state)
        - plan(turn_decision, understanding, stage_decision, user_state)
        """
        if kwargs:
            try:
        ...
        if len(args) == 5:
            ...
            # Route through the richer historical implementation if available.
            prev = globals().get('_Prev_ProjectNurturePlanner_4')
            if prev and hasattr(prev, 'plan'):
                try:
                    plan = prev.plan(self, understanding, stage_decision, turn_decision, project_decision, user_state)
                except TypeError:
                    plan = {}
            else:
                plan = {}
            if not isinstance(plan, dict):
                plan = {}
            if not plan:
                should_include = bool(getattr(project_decision, 'project_id', None) is not None and getattr(turn_decision, 'should_push_project', False))
                plan = {
                    'should_include_project_content': should_include,
                    'nurture_angle': 'single_clear_point' if should_include else None,
                    'max_project_points': 1 if should_include else 0,
                }
            project_window = getattr(turn_decision, 'project_window', None)
            pressure_guard = getattr(turn_decision, 'pressure_guard', None)
            project_strategy = getattr(turn_decision, 'project_strategy', None)
            if project_window == 'hold' or pressure_guard == 'strict':
                plan['should_include_project_content'] = False
                plan['angle'] = 'presence_over_project'
                plan['visibility'] = 'none'
                plan['project_window'] = project_window or 'hold'
            elif project_strategy == 'clarify_then_pause':
                plan['angle'] = 'clear_fact_then_pause'
                plan['visibility'] = 'light'
            elif project_window and 'project_window' not in plan:
                plan['project_window'] = project_window
            if 'angle' not in plan and plan.get('nurture_angle'):
                plan['angle'] = plan.get('nurture_angle')
            return plan

        if len(args) == 4:
            return super().plan(*args)

        raise TypeError(f'ProjectNurturePlanner.plan() unexpected args: {len(args)}')


_V52PrevTGAdminFormatter = TGAdminFormatter
class TGAdminFormatter(_V52PrevTGAdminFormatter):
    @staticmethod
    def format_dashboard(summary: dict) -> str:
        ops = summary.get('ops_counts', {}) or {}
        return (
            ...
            f"全局AI: {'开启' if summary.get('ai_global_enabled') else '关闭'}\n"
            f"开放队列: {summary.get('open_admin_queue_count', 0)}\n"
            f"高意向: {summary.get('high_intent_queue_count', 0)}\n"
            f"紧急处理: {summary.get('urgent_queue_count', 0)}\n"
            f"待恢复AI: {summary.get('pending_resume_count', 0)}\n"
            f"待审核回执: {summary.get('pending_review_receipt_count', summary.get('pending_receipt_count', 0))}\n"
            f"决策日志: {summary.get('decision_log_count', 0)}\n"
            f"项目锁定: {summary.get('locked_project_state_count', 0)}\n"
            f"标签锁定: {summary.get('locked_tag_count', 0)}\n\n"
            '用户分类:\n'
            f"- 新用户: {ops.get('new_user', 0)}\n"
            f"- 跟进用户: {ops.get('followup_user', 0)}\n"
            f"- 高意向用户: {ops.get('high_intent_user', 0)}"
    ...
    @staticmethod
    def format_queue(title: str, items: list[dict]) -> str:
        if not items:
            return f'📂 {title}\n\n暂无数据。'
        lines = [f'📂 {title}', '']
        for idx, item in enumerate(items[:20], start=1):
            lines.append(
                f"{idx}. 用户#{item.get('user_id', '-') } | 类型: {item.get('queue_type', '-') } | 优先级: {item.get('priority_score', '-') }"
            )
        return '\n'.join(lines)

    @staticmethod
    def format_receipts(items: list[dict]) -> str:
        if not items:
            return '🧾 待审核回执\n\n暂无待审核回执。'
        lines = ['🧾 待审核回执', '']
        for idx, item in enumerate(items[:20], start=1):
            lines.append(
                f"{idx}. 用户#{item.get('user_id', '-') } | 类型: {item.get('receipt_type', item.get('decision_type', '-')) }"
            )
        return '\n'.join(lines)


_V52PrevTGAdminMenuBuilder = TGAdminMenuBuilder
class TGAdminMenuBuilder(_V52PrevTGAdminMenuBuilder):
    @staticmethod
    def main_menu() -> dict:
        return {
            'text': '📊 管理后台\n请选择操作：',
            'reply_markup': [
                [
                    {'text': '📈 仪表盘', 'callback_data': 'adm:dashboard'},
                    {'text': '🔥 高意向客户', 'callback_data': 'adm:queue:high_intent'},
                ],
                [
                    {'text': '⚠️ 紧急处理', 'callback_data': 'adm:queue:urgent_handover'},
                    {'text': '⏳ 待恢复AI', 'callback_data': 'adm:queue:pending_resume'},
                ],
                [
                    {'text': '🧾 待审核回执', 'callback_data': 'adm:receipts:pending'},
                    {'text': '📂 全部队列', 'callback_data': 'adm:queue:all'},
                ],
                [
                    {'text': '🔄 刷新菜单', 'callback_data': 'adm:main'},
                ],
            ],
        }


_V52PrevTGAdminCallbackRouter = TGAdminCallbackRouter
class TGAdminCallbackRouter(_V52PrevTGAdminCallbackRouter):
    def _safe_business_account_id(self) -> int:
        return 1
    ...
    def handle(self, admin_chat_id: int, callback_data: str, operator: str = 'admin') -> None:
        ...
        if not parts or parts[0] != 'adm':
            ...
            return
        action = parts[1] if len(parts) > 1 else 'main'
        business_account_id = self._safe_business_account_id()
        ...
        if action == 'queue':
            ...
            q = None if queue_type == 'all' else queue_type
            items = self.dashboard_service.list_open_queue(business_account_id, q, 20)
            title_map = {
                'all': '全部队列',
                'high_intent': '高意向客户',
                'urgent_handover': '紧急处理',
                'pending_resume': '待恢复AI',
            }
            self.tg_sender.send_admin_text(admin_chat_id, TGAdminFormatter.format_queue(title_map.get(queue_type, queue_type), items))
            return
        if action == 'receipts':
            sub = parts[2] if len(parts) > 2 else 'pending'
            if sub == 'pending':
                items = self.admin_api_service.list_pending_review_receipts(business_account_id, 20)
                self.tg_sender.send_admin_text(admin_chat_id, TGAdminFormatter.format_receipts(items))
                return
        self.tg_sender.send_admin_text(admin_chat_id, f'⚠️ Unsupported callback: {callback_data}')


_V52PrevTGAdminHandlers = TGAdminHandlers
class TGAdminHandlers(_V52PrevTGAdminHandlers):
    def handle_admin_command(self, admin_chat_id: int, command_text: str, operator: str = 'admin') -> None:
        text = (command_text or '').strip().lower()
        if text in ('/admin', '/startadmin', 'admin', 'menu'):
            menu = TGAdminMenuBuilder.main_menu()
            self.tg_sender.send_admin_message(admin_chat_id, menu['text'], menu['reply_markup'])
            return
        self.tg_sender.send_admin_text(admin_chat_id, '⚠️ 未知的管理员命令，请使用 /admin')

    def handle_admin_text(self, admin_chat_id: int, command_text: str, operator: str = 'admin') -> None:
        return self.handle_admin_command(admin_chat_id, command_text, operator)


_v52_prev_build_app_components = build_app_components

def build_app_components(settings: Settings) -> dict[str, Any]:
    components = _v52_prev_build_app_components(settings)

    # Rebuild and rebind final V5/V5.1/V5.2 runtime chain on top of the stable base.
    strategy_optimizer = AdaptiveStrategyOptimizer()
    rhythm_engine = RelationshipRhythmEngine()
    maturity_engine = RelationshipMaturityEngine()
    memory_priority_resolver = MemoryPriorityResolver()
    ...
    humanization_controller = HumanizationController(optimizer=strategy_optimizer, damping_engine=damping_engine)
    memory_selector = MemorySelector(
        resolver=memory_priority_resolver,
        guard=ContinuityGuard(),
        anchor_engine=AnchorMemoryEngine(),
    )
    project_nurture_planner = ProjectNurturePlanner(window_evaluator=project_window_evaluator)
    ...
    tg_client = components.get('tg_client')
    admin_api_service = components.get('admin_api_service')
    dashboard_service = components.get('dashboard_service')
    if tg_client and admin_api_service and dashboard_service:
        tg_admin_callback_router = TGAdminCallbackRouter(admin_api_service, dashboard_service, tg_client)
        tg_admin_handlers = TGAdminHandlers(tg_admin_callback_router, tg_client)
        components['tg_admin_callback_router'] = tg_admin_callback_router
        components['tg_admin_handlers'] = tg_admin_handlers
