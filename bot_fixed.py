...
# =========================
# V5.2 compatibility and admin-chain convergence patch
# =========================

_Prev_ProjectNurturePlanner_5 = globals().get("_Prev_ProjectNurturePlanner_4", ProjectNurturePlanner)

class ProjectNurturePlanner(_Prev_ProjectNurturePlanner_5):
    """
    Compatibility wrapper:
    - supports legacy signature:
      plan(understanding, stage_decision, turn_decision, project_decision, user_state)
    - supports newer compact signature:
      plan(turn_decision, understanding, stage_decision, user_state)
    Always returns a normalized dict with both legacy and newer keys.
    """
    def plan(self, *args, **kwargs) -> dict[str, Any]:
        ...
        if len(args) == 5:
            ...
            raise TypeError(f"ProjectNurturePlanner.plan() received unsupported arguments: {len(args)} positional")

        if understanding is None or stage_decision is None or turn_decision is None or user_state is None:
            raise TypeError("ProjectNurturePlanner.plan() missing required normalized arguments")

        base: dict[str, Any] = {}
        # Prefer the stable 5-arg ancestor implementation if a project decision exists.
        if project_decision is not None:
            try:
                base = _Prev_ProjectNurturePlanner_5.plan(self, understanding, stage_decision, turn_decision, project_decision, user_state)  # type: ignore[misc]
            except TypeError:
                base = {}
            except Exception:
                base = {}
        else:
            # Compact fallback if no project_decision is available.
            should_push = bool(getattr(turn_decision, "should_push_project", False))
            base = {
                "should_include_project_content": should_push,
                "nurture_angle": getattr(turn_decision, "project_strategy", None) or "single_clear_point",
                "max_project_points": 1 if should_push else 0,
            }

        should_include = bool(base.get("should_include_project_content", False))
        project_window = getattr(turn_decision, "project_window", None) or "open"
        pressure_guard = getattr(turn_decision, "pressure_guard", None) or "normal"
        project_strategy = getattr(turn_decision, "project_strategy", None) or base.get("nurture_angle") or "single_clear_point"

        visibility = "light" if should_include else "none"
        angle = project_strategy
        ...
        if project_window == "hold" or pressure_guard == "strict":
            ...
            visibility = "light"
            angle = "clear_fact_then_pause"
        elif project_window == "soft_background":
            visibility = "light"
            angle = "reassure_then_light_value"
        elif project_window == "careful_open" and angle == "single_clear_point":
            visibility = "light"
            angle = "clear_fact_then_soft_bridge"
        elif getattr(turn_decision, "engagement_mode", None) == "reassure_and_hold":
            visibility = "light"
            angle = "reduce_pressure_then_clarify"
        elif getattr(turn_decision, "user_style_profile", None) == "logic_first":
            angle = "clear_fact_then_soft_bridge"
        elif getattr(turn_decision, "user_style_profile", None) == "emotion_first":
            angle = "reassure_then_light_value"

        max_points = int(base.get("max_project_points") or (1 if should_include else 0))
        if visibility == "none":
            max_points = 0
    ...
    def main_menu() -> dict:
        return {
            ...
            "reply_markup": [
                [{"text": "📈 仪表盘", "callback_data": "adm:dashboard"}, {"text": "🔥 高意向客户", "callback_data": "adm:queue:high_intent"}],
                [{"text": "⚠️ 紧急处理", "callback_data": "adm:queue:urgent_handover"}, {"text": "⏳ 待恢复AI", "callback_data": "adm:queue:pending_resume"}],
                [{"text": "🧾 待审核回执", "callback_data": "adm:receipts:pending"}, {"text": "📂 全部队列", "callback_data": "adm:queue:all"}],
...
class TGAdminCallbackRouter:
    ...
    def _send_queue(self, admin_chat_id: int, business_account_id: int, queue_type: str | None, title: str) -> None:
        rows = self.dashboard_service.list_open_queue(business_account_id, queue_type, 20) or []
        if not rows:
            self.tg_sender.send_admin_text(admin_chat_id, f"{title}\n\n暂无数据。")
            return
        lines = [title, ""]
        for idx, row in enumerate(rows[:20], 1):
            uid = row.get("user_id") or row.get("customer_user_id") or "-"
            conv = row.get("conversation_id") or "-"
            reason = row.get("reason") or row.get("queue_reason") or row.get("status") or "no_reason"
            lines.append(f"{idx}. 用户:{uid} | 会话:{conv} | {reason}")
        self.tg_sender.send_admin_text(admin_chat_id, "\n".join(lines))

    def _send_pending_receipts(self, admin_chat_id: int, business_account_id: int) -> None:
        rows = self.admin_api_service.list_pending_review_receipts(business_account_id, 20) or []
        if not rows:
            self.tg_sender.send_admin_text(admin_chat_id, "🧾 待审核回执\n\n暂无待审核回执。")
            return
        lines = ["🧾 待审核回执", ""]
        for idx, row in enumerate(rows[:20], 1):
            rid = row.get("id") or "-"
            rtype = row.get("receipt_type") or row.get("type") or "receipt"
            status = row.get("status") or "pending"
            user_id = row.get("user_id") or "-"
            lines.append(f"{idx}. 回执:{rid} | 类型:{rtype} | 用户:{user_id} | 状态:{status}")
        self.tg_sender.send_admin_text(admin_chat_id, "\n".join(lines))
    ...
    def handle(self, admin_chat_id: int, callback_data: str, operator: str = "admin") -> None:
        ...
        if action == "dashboard":
            ...
            text = TGAdminFormatter.format_dashboard(summary)
            text += f"\n\n决策日志: {summary.get('decision_log_count', 0)}"
            text += f"\n锁定项目状态: {summary.get('locked_project_state_count', 0)}"
            text += f"\n锁定标签: {summary.get('locked_tag_count', 0)}"
            text += f"\n待审核回执: {summary.get('pending_review_receipt_count', 0)}"
        ...
        if action == "queue":
            ...
            title_map = {
                "all": "📂 全部队列",
                "high_intent": "🔥 高意向客户",
                "urgent_handover": "⚠️ 紧急处理",
                "pending_resume": "⏳ 待恢复AI",
            }
            actual_queue_type = None if queue_type == "all" else queue_type
            self._send_queue(admin_chat_id, business_account_id, actual_queue_type, title_map.get(queue_type, "📂 队列"))
            return

        if action == "receipts":
            receipt_type = parts[2] if len(parts) > 2 else "pending"
            if receipt_type == "pending":
                self._send_pending_receipts(admin_chat_id, business_account_id)
                return

        menu = TGAdminMenuBuilder.main_menu()
        self.tg_sender.send_admin_message(
            admin_chat_id,
            f"⚠️ 暂不支持的管理员回调：{callback_data}\n\n已返回主菜单。",
            menu["reply_markup"],
        )
